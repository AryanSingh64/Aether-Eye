// app_httpd.cpp
// Based on Espressif CameraWebServer example, extended with /light/on and /light/off on port 80.
...
// Choose GPIO for light/relay.
// Note: On ESP32‑CAM AI-Thinker, the on-board flash LED is on GPIO4 and shares with SD; avoid SD if using it directly.
static const gpio_num_t LIGHT_GPIO = GPIO_NUM_4;

// Simple GPIO control for light (use HIGH/LOW per your wiring).
static void light_init() {
  ...
  gpio_config(&io_conf);
  gpio_set_level(LIGHT_GPIO, 0); // start OFF
}

static esp_err_t light_on_handler(httpd_req_t *req) {
  gpio_set_level(LIGHT_GPIO, 1); // active-high; invert if your driver is active-low
...
// ----- Stock example code below -----

// LED FLASH setup (used by UI controls)
...
static esp_err_t bmp_handler(httpd_req_t *req) {
  ...
  char ts[32]; snprintf(ts, 32, "%lld.%06ld", fb->timestamp.tv_sec, fb->timestamp.tv_usec);
  ...
  while (true) {
    ...
    if (!fb) {
      ...
      if (fb->format != PIXFORMAT_JPEG) {
        ...
        if (!jpeg_converted) { log_e("JPEG compression failed"); res = ESP_FAIL; }
      } else {
...
static esp_err_t cmd_handler(httpd_req_t *req) {
  ...
  if (!strcmp(variable, "framesize")) {
    ...
    led_duty = val; if (isStreaming) enable_led(true);
  }
#endif
...
static esp_err_t reg_handler(httpd_req_t *req) {
  char *buf = NULL; char _reg[32]; char _mask[32]; char _val[32];
  if (parse_get(req, &buf) != ESP_OK) return ESP_FAIL;
  ...
  free(buf);
  int reg = atoi(_reg); int mask = atoi(_mask); int val = atoi(_val);
...
static esp_err_t greg_handler(httpd_req_t *req) {
  char *buf = NULL; char _reg[32]; char _mask[32];
  if (parse_get(req, &buf) != ESP_OK) return ESP_FAIL;
  if (httpd_query_key_value(buf, "reg", _reg, sizeof(_reg)) != ESP_OK ||
      httpd_query_key_value(buf, "mask", _mask, sizeof(_mask)) != ESP_OK) {
    free(buf); httpd_resp_send_404(req); return ESP_FAIL;
  ...
  free(buf);
  int reg = atoi(_reg); int mask = atoi(_mask);
  sensor_t *s = esp_camera_sensor_get();
  int res = s->get_reg(s, reg, mask);
  if (res < 0) return httpd_resp_send_500(req);
  log_i("Get Register: reg: 0x%02x, mask: 0x%02x, value: 0x%02x", reg, mask, res);
  char buffer[20]; const char *val = itoa(res, buffer, 10);
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
...
void startCameraServer() {
  httpd_config_t config = HTTPD_DEFAULT_CONFIG();
    // increase socket budget and enable keep-alive to let server handle multiple clients
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    /* --- PERFORMANCE MOD ---
     * Removed all unused web UI handlers to save RAM and CPU.
     * We ONLY need /light/on and /light/off for the Python server.
    */
    httpd_register_uri_handler(camera_httpd, &index_uri);   // DELETED
    httpd_register_uri_handler(camera_httpd, &cmd_uri);     // DELETED
    httpd_register_uri_handler(camera_httpd, &status_uri);  // DELETED
    httpd_register_uri_handler(camera_httpd, &capture_uri); // DELETED
    httpd_register_uri_handler(camera_httpd, &bmp_uri);     // DELETED
    httpd_register_uri_handler(camera_httpd, &xclk_uri);    // DELETED
    httpd_register_uri_handler(camera_httpd, &reg_uri);     // DELETED
    httpd_register_uri_handler(camera_httpd, &greg_uri);    // DELETED
    httpd_register_uri_handler(camera_httpd, &pll_uri);     // DELETED
    httpd_register_uri_handler(camera_httpd, &win_uri);     // DELETED

    // Register custom light control endpoints on port 80
    light_init();
    static const httpd_uri_t light_on_uri  = { .uri="/light/on",  .method=HTTP_GET, .handler=light_on_handler,  .user_ctx=NULL };
    static const httpd_uri_t light_off_uri = { .uri="/light/off", .method=HTTP_GET, .handler=light_off_handler, .user_ctx=NULL };
    httpd_register_uri_handler(camera_httpd, &light_on_uri);
    httpd_register_uri_handler(camera_httpd, &light_off_uri);
  }

  // Start separate stream server on next port (default 81)
